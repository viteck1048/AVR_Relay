...
void prnt_1x7(unsigned short int type, byte y, byte x)
{
	...
	switch(type) {
		case 0b1100:{
			if(((obscht_r + tek_r) / 3600) < 10000) {
				...
				if(((obscht_r + tek_r) / 3600) / 10000 != 0)
					dsplbuf.prnt(y, x, "%1d", ((obscht_r + tek_r) / 3600) / 10000, 1);
				pp = dsplbuf.prnt(y, pp, NULL, (tek_r % 2 == 1 || tmr_ == 0) ? (byte)'.' : 0x20, 0);
				dsplbuf.prnt(y, pp, "%d", ((obscht_r + tek_r) / 360) % 10, 1);
			}
			else {
				pp = dsplbuf.prnt(y, x + 3, "%04u", ((obscht_r + tek_r) / 3600) % 10000, 1);
				if(((obscht_r + tek_r) / 3600) / 10000 != 0)
					dsplbuf.prnt(y, x, "%3u", ((obscht_r + tek_r) / 3600) / 10000, 1);
			}
			break;
		}
		case 0b1101:{
		...
		case 0b1110:{
			...
			pp = dsplbuf.prnt(y, pp, NULL, digitalRead(OUT_1) == D_O_ON ? 0xff : 0xdb, 0);
			pp = dsplbuf.prnt(y, pp, NULL, digitalRead(OUT_2) == D_O_ON ? 0xff : 0xdb, 0);
			dsplbuf.prnt(y, pp, NULL, digitalRead(OUT_3) == D_O_ON ? 0xff : 0xdb, 0);
			break;
		}
		case 0b0000:{
			rv[0].prnt(y, x);
